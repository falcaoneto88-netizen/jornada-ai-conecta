...
export default defineTool({
  ...
  description: "Lista os registos de auditoria mais recentes da organização (ações, entidades e datas).",
  inputSchema: {
  ...
  handler: async ({ limit }, ctx) => {
    ...
    const { data, error } = await supabase
      .from("audit_logs")
      .select("action,entity,entity_id,actor_name,metadata,created_at")
