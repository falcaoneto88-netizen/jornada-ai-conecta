-- Controlos explícitos e separados (escrita remota / canal de acolhimento) para o
-- administrador autenticado da própria organização. Sem parâmetro de organização:
-- a organização vem sempre da sessão. Só configura: nunca executa filas, nunca
-- reprocessa pendências e nunca altera o ledger nem estados incertos.
...
BEGIN
  ...
  IF _state = 'ligado' THEN
    -- Só habilita com destino fixo verificado, binding da própria organização e
    -- ligação ao GoHighLevel conectada com escrita global ativa.
    ...
    END IF;
  ELSE
    -- Desligar nunca depende de credenciais nem da ligação externa.
    IF _scope = 'remote_write' THEN
